...
ins_a_sum_sq_vec <- rep(NA, n)

for (i in 1:a) {
  
  ins_a_sum_sq <- y[x_a == unique(x_a)[i]]
  ins_a_sum_sq <- (ins_a_sum_sq - grp_means_a$mean[i]) ^ 2
  ins_a_sum_sq_vec[(((i - 1) * n) + 1):((i * n))] <- ins_a_sum_sq
  
}

# calculate sums of squares within b
ins_b_sum_sq_vec <- rep(NA, n)

for (i in 1:a) {
  
  ins_b_sum_sq <- y[x_b == unique(x_b)[i]]
  ins_b_sum_sq <- (ins_b_sum_sq - grp_means_b$mean[i]) ^ 2
  ins_b_sum_sq_vec[(((i - 1) * n) + 1):((i * n))] <- ins_b_sum_sq
  
}

# compile sum of squares vector overall
# TODO: Do this properly
sum_sq <- c("A"  = sum((grp_means_a$mean  - gm) ^ 2 * n), # should this be time n or a?
            "B"  = sum((grp_means_b$mean  - gm) ^ 2 * n),
            "AB" = NA,
            "E"  = NA, # something with the 2 ind_x_sum_sq_vecs
            "T"  = sum((y - gm) ^ 2))
...
non_int_vec <- c(1, 2, 4, 5)

# assemble table
aov_tab <- data.frame("Sum Sq"  = sum_sq[non_int_vec],
                      "df"      = dfs[non_int_vec],
                      "Mean Sq" = mean_sq[non_int_vec],
                      "F val"   = f_val[non_int_vec],
                      row.names = c("Environment", "Family", "Error", "Total"))
